...
namespace LearnABP.Tests
{
    public abstract class LearnABPTestBase : AbpIntegratedTestBase<LearnABPTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected LearnABPTestBase()
        {
        ...
        protected void UsingDbContext(Action<LearnABPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LearnABPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LearnABPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LearnABPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LearnABPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LearnABPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LearnABPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LearnABPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LearnABPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LearnABPDbContext, Task<T>> func)
        {
            T result;
